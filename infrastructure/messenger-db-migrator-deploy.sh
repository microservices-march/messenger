...
POSTGRES_USER=$(curl -X GET "http://${CONSUL_HOST}:${CONSUL_PORT}/v1/kv/messenger-db-migrator-user?raw=true")
...
PGPASSWORD=$(curl -X GET "http://${CONSUL_HOST}:${CONSUL_PORT}/v1/kv/messenger-db-migrator-password-never-do-this?raw=true")
